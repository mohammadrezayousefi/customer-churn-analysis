...
st.set_page_config(page_title="پلتفرم هوشمند نگهداشت", layout="wide")
...
FEATURE_TRANSLATION = {
    'هزینه_ماهیانه_تومان': 'هزینه ماهیانه بالا', 'سابقه_سیم‌کارت_ماه': 'سابقه سیمکارت',
...
ACTION_CATALOG = {
    # -- هزینه ماهیانه --
    'هزینه_ماهیانه_تومان_1': 'ترکیب اینترنت و مکالمه با ۳۰٪ هزینه کمتر — یک بسته، همه چیز.',
    'هزینه_ماهیانه_تومان_2': 'آخر هفته‌ها اینترنت مصرف کن، هزینه‌ای پرداخت نکن.',
    'هزینه_ماهیانه_تومان_3': '۲۰٪ بعد از هر خرید بسته پول برمی‌گرده به کیف حسابت —با خرید بعدی پس انداز کن.',
    'هزینه_ماهیانه_تومان_4': '  اینترنت شبانه نامحدود — دانلود کن بدون نگرانی از حجم.',
    'هزینه_ماهیانه_تومان_101': 'بسته دلخواهت رو خودت بساز! حجم و مدت‌زمان را خودت انتخاب کن تا هزینه‌هات کاملاً مدیریت بشه.',
    'هزینه_ماهیانه_تومان_102': 'اینترنت شبانه ۱۱ تا ۱ بامداد برگشت! مخصوص شما برای دانلودهای سنگین با کمترین هزینه.',
    'هزینه_ماهیانه_تومان_103': 'بسته شبانه یک‌روزه فعال شد — فقط برای امشب، با قیمتی که باورت نمی‌شه.',
    ...
    'سابقه_سیم‌کارت_ماه_4': 'مسابقه زنده می‌بینی؟تا یک ماه ترافیک تماشا از بسته‌ات کم نمی‌شود.',
    ...
    'نوع_سیم‌کارت_اعتباری_3': 'شارژ خریدی؟خط دائمی کن هر بار دو برابر شارژ کن  ',
    ...
    # -- فضای ابری --
    'فضای_ابری_اپراتور_خیر_1': '۲۰ گیگ فضای ابری همین الان برای تو فعال می‌شود — عکس‌هایت را از دست نده.',
    ...
    'استفاده_اپلیکیشن_اپراتور_خیر_103': 'کوپن‌های همراه اول اینجاست! در این برنامه‌ها و فروشگاه‌ها، تخفیف‌های ویژه منتظر شماست.',
    ...
    'بسته_رومینگ_بین‌الملل_خیر_1': 'در حال برنامه‌ریزی سفر هستید؟ بسته‌های رومینگ اقتصادی ما را با تخفیف ویژه رزرو کنید.',
    'بسته_رومینگ_بین‌الملل_خیر_2': 'سیم‌کارت برای استفاده بین‌المللی ارتقا بده. (ارسال پیامک اطلاع‌رسانی تعرفه‌های جدید).'
}
    ...
    with col_reasons:
        ...
        if churn_prob < 0.4:
            ...
            if len(possible_actions) >= 2:
                ...
                st.markdown(f'<div class="action-box">راهکار عمومی:<br>{fallback}</div>', unsafe_allow_html=True)
            ...
            if f1.button("تایید و اجرای کمپین", key=f"yes_{target_user_id}"):
                ...
                st.toast("بازخورد مثبت در سیستم MLOps ثبت شد.")
            if f2.button("راهکار مناسب نیست", key=f"no_{target_user_id}"):
                ...
                st.toast("بازخورد منفی جهت بازآموزی مدل ثبت شد.")
        st.markdown('</div>', unsafe_allow_html=True)
...
with tab2:
    ...
    if uploaded_file is not None:
        ...
        if st.button("شروع پردازش و شناسایی ریسک"):
